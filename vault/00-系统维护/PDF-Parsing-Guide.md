...
对质量路由标记为 `enhanced_parse_queued` 的页面，可以另行运行 MinerU 单页增强器。它生成独立候选，不能回写本页基础文本；即使替换字符减少或生成了 LaTeX，仍需对照原页检查数学含义和阅读顺序。详见 [[00-系统维护/Enhanced-Parsing-Guide|本地公式增强解析指南]]。
