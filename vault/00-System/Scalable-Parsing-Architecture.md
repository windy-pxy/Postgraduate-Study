...
Phase 2D-0 在这层架构之上增加元数据批次规划。它读取 manifest、质量路由、资源限制和队列状态，将页面分为基础候选、未来增强、人工审核、资源推迟、已在队列和不支持六类。规划结果不等于执行授权，也不会改变页面状态。操作说明见 [[00-System/Batch-Parsing-Guide|大规模资料批次规划指南]]。
