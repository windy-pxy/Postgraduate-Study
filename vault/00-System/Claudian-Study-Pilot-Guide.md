...
本试点把 Obsidian 中的 Claudian 连接到本机已有 Codex CLI，用于读取和引用学习资料。它不改变来源审核状态，也不把待审核候选当作正式知识。项目规则位于 vault 根目录的 [[AGENTS|study_readonly_pilot]]。
...
当前只读会话不能写文件。只有用户明确说“生成草稿”并另行授权受控写入后，才可在 `03-Knowledge-Notes/AI-Drafts/` 新建草稿。候选内容仍需人工对照原页；Claudian 无权把内容标为 accepted，也无权把草稿移入正式笔记区。

[[00-System/Home|返回首页]] · [[00-System/Local-Retrieval-Guide|本地检索指南]] · [[00-System/Validation-Guide|验证说明]]
