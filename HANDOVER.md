...
- 已提交阶段：Phase 0 → 1 → 2A → 2B → 2C → 2D-0 → 2E → 3（本地检索），HEAD = `85ac896`。
- **Phase 3A（Claudian + Codex read-only 试点）代码/文档已完成但未提交**：
  - 新增：`scripts/validate_claudian_pilot.py`、`tests/test_claudian_pilot.py`、`vault/AGENTS.md`、`vault/00-System/Claudian-Study-Pilot-Guide.md`、`vault/03-Knowledge-Notes/AI-Drafts/`
  - 修改：`.gitignore`、`AGENTS.md`、`README.md`、`scripts/validate_vault.py`、`tests/test_validate_vault.py`、`vault/00-System/Home.md`、`vault/00-System/Validation-Guide.md`
...
## 2. 本次代管期间 WorkBuddy 的变更（需用户确认后才可 commit）
...
1. 是否 commit Phase 3A 未提交改动（规则 11：必须用户明确确认）。
...
1. 读本文件 + `logs/autonomous-progress.json` + `review-queue/` 两个审核文件。
2. 与用户确认第 4 节待决策事项。
3. 若 Phase 3A 已提交且 Codex 可用：按 `vault/00-System/Claudian-Study-Pilot-Guide.md` 在 Obsidian 内跑 3 条 UI 试问（正式问答应返回 accepted 内容或"暂无已审核资料"，候选预览须 basic/enhanced 分离）。
4. 后续路线：Phase 4（多模型适配、带来源页码的问答）；增强解析器对试卷类双栏密排页面的结构性问题（丢题干、粘连）应记录为解析器评估输入，不要直接重跑覆盖既有候选。
